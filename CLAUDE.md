...
4. Real hosting decision — free (Netlify/GitHub Pages/Cloudflare Pages) vs. paid custom domain.
...
6. ~~Step 4 — single-device login enforcement~~ — **built 2026-07-26, SQL run by her**. End-to-end test blocked on real hosting (item 4 above).
...
8. Supabase dashboard hardening — two separate pieces: (a) leaked-password-protection toggle — a one-click flip in her Supabase dashboard (Authentication → Policies), not code, she can do this any time; (b) tightening Edge Function CORS — genuinely blocked on knowing the real domain, deferred until hosting is picked.
