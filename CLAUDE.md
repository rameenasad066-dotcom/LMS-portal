...
8. Supabase dashboard hardening — two separate pieces: (a) ~~leaked-password-protection toggle~~ — **checked 2026-07-27: not available**, it's a Supabase Pro-plan feature and her project is on the free tier; skipped, not a launch blocker (bumped "Minimum password length" from 6→8 instead, which is free); (b) ~~tightening Edge Function CORS~~ — **done 2026-07-27**: she deployed to Netlify (`https://studywithrameenlms.netlify.app`), and both `create-student` and `reset-student-password` now check the request's `Origin` header against an allowlist (the Netlify domain + `localhost:5501`/`5500` for local dev) instead of `Access-Control-Allow-Origin: *`. **She still needs to re-paste the updated `index.ts` into the Supabase Edge Functions dashboard and redeploy both functions** — editing the file in this repo does not update the already-deployed function.
