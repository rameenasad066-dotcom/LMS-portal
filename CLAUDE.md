...
│   ├── hp-quotes.json— Harry Potter quote of the day on the student dashboard (owner-editable)
...
**Study-buddy owl easter egg (`owl.js`):** sits pre-perched from page load (no flight-in, removed 2026-07-27 at her request) — on the portals it sits in-flow at the bottom of the sidebar (never overlaps UI); on the gateway it sits on the card's red top border. Click → hop + a speech bubble with a **study fact** (syllabus fact / examiner tip / key date). Facts come from `data/owl-facts.json` (owner-editable; fetched by `owl.js` itself, shuffled, no immediate repeats); if that file can't load, the owl falls back to a few built-in facts so it's never silent (no toast — it fails quietly). Owl art is fully neutral (white/gray/ink) — big dark round eyes with white catch-lights, neutral beak & talons (no amber anywhere on the bird), never green, per the color rule. The cute redesign is deliberately soft/rounded (no spiky wings or sharp tufts). Only folded (perched) wings remain — the spread-wing flight art, the letter/wax-seal prop, and the flap/bob/mail-swing animations were all deleted along with the flight-in.
...
| `--surface-2` | `#F3F1F1` | Subtle raised/hover/muted surface (table-row hover, empty-note, muted pill) |
| `--border-soft` | `#F1EFEF` | Faint hairline borders |
| `--ink-red` | `= --red-deep` | Red used as **text on tint** (kept separate from `--red-deep`'s hover-background role — see dark mode) |
| `--ink-green` | `= --green-deep` | Green used as **text on tint** |

**Light / dark mode (added 2026-07-28 at Rameen's request):** the whole design system is token-driven, so dark mode is a `:root[data-theme="dark"]` override of the palette variables — every component follows automatically. Applied by `theme.js` (toggle, persisted in localStorage `swr_theme`) plus a tiny inline `<head>` no-flash script on every page that sets `data-theme` before first paint. The toggle is a `.theme-toggle` `.icon-btn` in each portal's topnav and a `.theme-toggle.floating` button on the gateway/login pages; the sun/moon icon swap is pure CSS. **Key correctness detail:** `--red-deep`/`--green-deep` are used in *two* opposite roles — darker **hover backgrounds** (need white text, so must stay dark) and **red/green text on light tints** (must become *light* in dark mode). These were decoupled: `--red-deep`/`--green-deep` keep the hover-background role; new `--ink-red`/`--ink-green` carry the text-on-tint role (every `color: var(--red-deep|green-deep)` was migrated to them). In dark mode `--ink-red`/`--ink-green` go light while the hover backgrounds stay dark. `color-scheme` is set per theme so native date/file pickers and scrollbars follow. All text/background pairs were WCAG-checked in both themes (AA-passing); `--red` is brightened in dark (`#E2434E`) so it stays legible as an accent/text on dark surfaces, `--green` kept at its light value so white-on-green fills don't regress. Every hardcoded off-white (`#FDFAFA`/`#F1F1F2`/etc.) was converted to `--surface-2`/`--border-soft`; a few hardcoded decorative tints (gateway bg, `.lec-thumb`, `.wt-paper:hover`, `.welcome-banner`) have explicit `[data-theme="dark"]` overrides at the end of `style.css`.
...
**8 nav views:** `dashboard` · `uploads` (labelled "Upload Notes") · `assignments` · `weekly-test` (labelled "Weekly Test", added 2026-07-27) · `students` · `attendance` · `scoreboard` · `settings`. One non-navlinked router view also exists: `student-report` (reached by clicking a student in the roster). (The old fake `grades` "Grade Distribution" view was deleted 2026-07-22.)
...
```
(The fake `UPLOADS` array was deleted in the 2026-07-22 dashboard rebuild — stale reference here until 2026-07-23.) `on26.meta`'s hardcoded "Next deadline: Diagnostic Quiz 2 · 15 Jul" placeholder (never wired to real data) was removed 2026-07-27 at her request — `meta` is now an empty string for that cohort; `mj27`'s "Classes begin 10 Aug 2026" line was left as-is since it's real, not a leftover placeholder.
...
**Weekly Test (real, Supabase-backed — rebuilt 2026-07-27):** replaces the old `data/current.json` + WhatsApp-submission flow entirely, which she never had a UI for at all (she'd have had to ask for the JSON file to be hand-edited each week). New **Weekly Test** nav item/view, `teacher-weekly-test.js`: post a test (title, PDF upload to new private bucket `weekly-tests`, and an "uploads close at" datetime picker she sets herself per test) to the active cohort; open a posted test to see every student's upload status and the **exact timestamp** they uploaded (no marks entry here — she chose to keep this feature to just collecting uploads + timestamps, grading a weekly test still happens via a separate "Test" type Assignment if she wants it graded in-portal). Table `weekly_tests` (cohort_id, title, pdf_path, closes_at) + `weekly_test_submissions` (weekly_test_id, student_id, file_paths, submitted_at) — `supabase/migrations/weekly-tests.sql`. **The cutoff is enforced server-side**, not just hidden in the UI: the submissions table's insert RLS policy checks `now() < weekly_tests.closes_at` directly in Postgres, so a student can't bypass it by editing the page — this was the actual point of the rebuild ("so the students don't cheat"). Student side: `student-weekly-test.js` (`renderStudentWeeklyTest()`, called from `auth-guard.js`) lists posted tests newest-first, each with a signed-URL PDF download, and either an upload form (before the cutoff, unsubmitted), an "Uploaded at HH:MM" confirmation (after submitting), or an "Uploads are closed" message (after the cutoff, never submitted) — reuses the existing `.asg-card`/`.asg-upload-form`/`.deadline-card` CSS, no new classes needed. Student photo uploads reuse the existing `submissions` storage bucket (path `<uid>/wt-<weekly_test id>/...`) since its RLS is already keyed to the uploader's own uid regardless of the second path segment.
...
**Dashboard widgets:** stat row · Syllabus Tracker rings (SVG `stroke-dashoffset`, red; turns green with "Complete ✓" ONLY at 100% — success state) · Leaderboard mini podium (`.podium.mini`, shares podium markup + rise animation with the scoreboard) · Announcement Board (`.ann-list`; tag colors: pinned = red fill, action = blush, info = neutral). **Welcome banner rebuilt 2026-07-27** — the old "Welcome back, {name} / Next Live Class: Tonight 8:30 PM" text (with no join button — live-class links were shared via WhatsApp) is gone entirely, replaced at her request with a Harry Potter **quote of the day** (`data/hp-quotes.json`, owner-editable): `quoteOfTheDay()` in `data.js` picks deterministically off the day-number so every student sees the same quote on a given day and it changes the next, no per-student randomness. `renderQuoteBanner()` in `student.js` fills `#hpQuoteText`/`#hpQuoteSpeaker`. The `.welcome-banner` has a red gradient background; its `h2` needed an explicit `color:#fff` (2026-07-28) because the global `h1,h2,h3 { color: var(--ink) }` rule was overriding the inherited white, rendering the quote black-on-red.
...
**Weekly Test (`weekly` view, renders into `#weeklyArea`):** rebuilt 2026-07-27 — see the Teacher Portal's Weekly Test section above for the full design (real Supabase tables, hard server-side cutoff enforcement). `student-weekly-test.js` lists posted tests newest-first with a signed-URL PDF download and, per test, either an upload form (before the cutoff), an "Uploaded at HH:MM" confirmation, or an "Uploads are closed" message. The old `current.json`-driven version (single global test, WhatsApp-only submission, no upload UI at all) is fully retired — the file is deleted and `CURRENT`/`loadData()`'s fetch of it are gone from `data.js`.
...
**In progress:**
11. Weekly Test rebuild — **built 2026-07-27, SQL not yet run.** She needs to run `supabase/migrations/weekly-tests.sql` in the Supabase SQL editor before posting a weekly test or uploading anything will work (new tables `weekly_tests`/`weekly_test_submissions` + storage bucket `weekly-tests` don't exist until then). Once run, this is ready to use — no further code changes needed.
...
8. Supabase dashboard hardening — two separate pieces: (a) ~~leaked-password-protection toggle~~ — **checked 2026-07-27: not available**, it's a Supabase Pro-plan feature and her project is on the free tier; skipped, not a launch blocker (bumped "Minimum password length" from 6→8 instead, which is free); (b) tightening Edge Function CORS — **done 2026-07-27, redeployed 2026-07-27, but the allowlist now points at the WRONG domain** since she renamed the Netlify site 2026-07-28 (`studywithrameenlms` → `studywithrameen-lms`). Code updated the same day to the new domain — **she needs to re-paste both `index.ts` files into the Supabase dashboard and redeploy again** before students on the new URL can use Add Student / Reset Password (this doesn't block plain student login, which never goes through these functions).
9. ~~Supabase Redirect URLs~~ — **needs re-doing 2026-07-28**: she confirmed this for the OLD domain on 2026-07-27, but after the Netlify rename she needs to update Site URL + Redirect URLs in Supabase Authentication settings to `https://studywithrameen-lms.netlify.app` or password-reset emails will link to the dead old domain.
...
10. ~~Replace placeholder quiz questions~~ — **all real now (2026-07-26)**: History (3 quizzes), Geography (1 quiz), and Islamiyat (5 quizzes — Prayer, Hajj, Zakat, Fasting, Angels) are all drafted from her own notes into `data/quizzes.json`. **Islamiyat still needs her religious-accuracy review before it's considered final** — flag this until she confirms. Replacing placeholder owl facts in `data/owl-facts.json` is still outstanding. (The old "drop the weekly-test PDF into `tests/`" step no longer applies — weekly test PDFs are uploaded directly through the Teacher Portal's Weekly Test page now, see item 11 above.)
